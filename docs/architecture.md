...
web/
  ...
  apps/miniapp/               # React + Vite SPA (MiniApp + web portal)
...
- **Telegram MiniApp** — TWA authenticated by Telegram init-data.
- **Web portal** — public browser client (invite code + email/password
  registration, email verification); same SPA, served by `frontend` at `/`.
...
- Payments: gateways (CryptoBot, CrystalPay, Platega, APay) and Google Play IAP
  create invoices via `packages/payments`; confirmations hit the bot's webhook
  endpoints, which verify signatures (`payments.signatures`) and deliver the
  subscription.

## Networking

Two segmented Docker networks (see [deployment.md](deployment.md)):

- **backend-network** (external) — the edge; the reverse-proxy nginx and the
  app backends + `frontend` live here.
- **data-network** (internal, no gateway) — PostgreSQL + the backends that talk
  to it. Postgres is **not** reachable from the edge or the internet.
